...
magnitude_spikes <- seq(mu_u_NR, mu_u_R * 3,length.out=3)        # Multiplicative magnitudes
base_mu_u_NR <- mu_u_NR
base_mu_u_R  <- mu_u_NR  * 5
N <- 50e4                               # Number of cells
n_samples <- 200                       # Number of samples
w <- 0.5                                 # weight for responders (example)
run <- 1 #using 3 because we want it to run faster
#Storage
results_auc <- data.frame()
results_pseudo_stim <- data.frame()
results_pseudo_unstim <- data.frame()
for (spike in spike_props) {
  for (magnitude in magnitude_spikes) {
    cat("Running spike =", spike, "magnitude =", magnitude, "\n")
    ...
    alpha_unstim <- param_values[1, "alphas.1"]
    beta_unstim  <- param_values[1, "alphas.0"]
    # Pseudo-trials ratio 
     pseudo_ratio_stim <- (alpha_stim + beta_stim - 2) / N
     pseudo_ratio_unstim <- (alpha_unstim + beta_unstim - 2) / N
    ...
    results_auc <- rbind(results_auc, 
                         data.frame(spike_props = spike,
                                    magnitude_spikes = round(magnitude,6),
                                    AUC = auc_val))
    results_pseudo_stim <- rbind(results_pseudo_stim,
                            data.frame(spike_props = spike,
                                       magnitude_spikes = round(magnitude,6),
                                       PseudoRatio = pseudo_ratio_stim
                                       ))
    results_pseudo_unstim <- rbind(results_pseudo_unstim,
                            data.frame(spike_props = spike,
                                       magnitude_spikes = round(magnitude,6),
                                       PseudoRatio = pseudo_ratio_unstim))
  }
}
# Plot  AUC vs Proportion of Spikes 
plot_auc <- ggplot(results_auc, aes(x = spike_props, y = AUC, color = factor(magnitude_spikes))) +
  geom_line(na.rm=TRUE) +
  labs(title = "AUC vs Proportion of Spikes",
       x = "Proportion of Spikes",
       y = "AUC",
       color = "Magnitude of Noise") +
  theme_bw()+cowplot::theme_cowplot()
# Plot Pseudo-cells ratio vs Proportion of Spikes 
plot_pseudo_stim <- ggplot(results_pseudo_stim, aes(x = spike_props, y = PseudoRatio, color = factor(magnitude_spikes))) +
  geom_line(na.rm=TRUE) +
  labs(title = "Stimulated(Activated):Pseudo-trials ratio vs Proportion of Spikes",
       x = "Proportion of Spikes",
       y = "Pseudo-trials/Real cells",
       color = "Magnitude of Noise") +
  theme_bw()+cowplot::theme_cowplot()
plot_pseudo_unstim <- ggplot(results_pseudo_unstim, aes(x = spike_props, y = PseudoRatio, color = factor(magnitude_spikes))) +
  geom_line(na.rm=TRUE) +
  labs(title = "Unstimulated(Resting):Pseudo-trials ratio vs Proportion of Spikes",
       x = "Proportion of Spikes",
       y = "Pseudo-trials/Real cells",
       color = "Magnitude of Noise") +
  theme_bw()+cowplot::theme_cowplot()
...
ggsave("plots/plot_pseudo_stim.png", plot = plot_pseudo_stim, width = 6, height = 4, dpi = 300)
ggsave("plots/plot_pseudo_unstim.png", plot = plot_pseudo_unstim, width = 6, height = 4, dpi = 300)
