...
c_NR_list<-c(26345)
mu_u_NR <- c(0.004)          # Means for non-responders (unstimulated)
mu_u_R  <- c(0.004)    # Means for responders (unstimulated), let it be the same
            ...
            for (w in w_list) {
                for (f in f_list) {
                    for (c_R in c_R_list) {
                        ...
                        print(paste("Parameters: n_samples =", n_samples, ", N =", N, ", w =", w, ", f =", f, ", c_R =", c_R,", c_NR =", c_NR))
                        # Store results from multiple runs
                        all_roc_mimosa <- list()
                        all_roc_fisher <- list()
                        all_roc_lrt <- list()
                        all_auc_fisher <- numeric(n_runs) 
                        ...
                        for(run in 1:n_runs) {
                            ...
                            dat <- get_sim_data(n_samples, w, params, N )
                            # Fit MIMOSA model
                            ...
                            roc_result <- ROC_curve(fit, n_samples, dat)
                            roc_fisher <- lrt_curve(dat)
                            #roc_lrt <-lrt_curve(data)
                            all_roc_mimosa[[run]] <- roc_result
                            all_roc_fisher[[run]] <- roc_fisher$roc_data
                             all_auc_fisher[run] <- roc_fisher$auc
                            #all_roc_lrt[[run]] <- roc_lrt
                        }
                        # Average the curves
                        avg_roc_mimosa <- average_roc_curves(all_roc_mimosa)
                        avg_roc_fisher <- average_roc_curves(all_roc_fisher)
                        # Mean AUC across runs
                        auc_mimosa <- mean(sapply(all_roc_mimosa, function(x) x$auc), na.rm = TRUE)
                      auc_fisher <- mean(all_auc_fisher, na.rm = TRUE)
                        # Plot averaged curves
                        p <- plot_curves(avg_roc_mimosa, avg_roc_fisher, n_samples, auc_mimosa, auc_fisher)
