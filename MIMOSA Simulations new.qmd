...
magnitude_spikes <- c(0.5, 2, 3)         # Multiplicative magnitudes
...
base_mu_u_R  <- mu_u_R
options(expressions = 500000)  #to solve C stack error
for (spike in spike_props) {
  for (magnitude in magnitude_spikes) {
    ...
    if (n_spike > 0) {
      if (rbinom(1, 1, 0.5) == 1) {
        mu_u_NR <- mu_u_NR * magnitude 
      } else {
        mu_u_R <- mu_u_R * magnitude
      }
      rand_params <- calculate_params(mu_u_NR, mu_u_R, f, c_u, c_s)
      data_all <- rbind(dat$data, rand_dat$data)
  responder_all <- c(dat$responder_status, rand_dat$responder_status)
    } else {
      data_all <- dat$data
    ...
    df <- bind_cols(pheno, as_tibble(prob_mat)) |>
...
prob_scores <- df_unique$mimosa_prob
