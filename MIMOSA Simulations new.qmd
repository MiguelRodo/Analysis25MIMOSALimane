...
, ",n_samples =", n_samples_val, 
                    ...
                    # Generate data
                    params <- calculate_params(mu_u_NR, mu_u_R, f_val, c_u,c_s)
...
We then fit MIMOSA using both the EM and MCMC methods, and assess performance using the AUC (ability to identify the responders) and the pseudo-trials ratio (measure of strength of prior information learned by MIMOSA about the data).
...
# Parameters
run_fast <- FALSE
spike_props <- c(0, 0.05, 0.1, 0.2) 
magnitude_spikes <- c(mu_u_NR, mu_u_NR * 5, mu_u_NR * 10, mu_u_NR * 20)
N <- 1e4 #em is heavy , originally is 1e4 or 2e4                              
n_samples <- 50                        
w <- 0.5                                
n_runs <- 10
mtd_vec <- "EM"  # EM only
  ...
  for (spike in spike_props) {
    ...
    for (magnitude in magnitude_spikes) {
      ...
      for (run in seq_len(n_runs)) {
        ...
        if (n_spike > 0) {
          rep_ind <- sample(nrow(data_all), size = n_spike, replace = FALSE)
        ...
        fit <- try(fit_MIMOSA(E, mtd = mtd), silent = TRUE)
        if (inherits(fit, "try-error")) {
          ...
          results_pseudo <- rbind(results_pseudo,
                                  data.frame(spike_props = spike,
                                            magnitude_spikes = magnitude,
                                            PseudoRatioStim = NA_real_,
                                            PseudoRatioUnstim = NA_real_,
                                            method = mtd,
                                            run = run))
        ...
        if (em_failure) {
          ...
          beta_uns <- uns_vec_par[1]
          alpha_uns <- uns_vec_par[2]
          pseudo_ratio_uns <- (alpha_uns + beta_uns - 2) / N
        }
...
for (mtd in mtd_vec) {
  ...
  plot_tbl_auc <- results_auc |>
    dplyr::filter(method == mtd) |>
     dplyr::mutate(magnitude_spikes = round(magnitude_spikes, 7)) |>
    dplyr::group_by(spike_props, magnitude_spikes) |>
    dplyr::summarise(
      AUC_MIMOSA = mean(AUC, na.rm = TRUE),
    ...
    dplyr::mutate(AUC_Type = gsub("AUC_", "", AUC_Type))

  plot_auc <- ggplot(plot_tbl_auc, aes(
    x = spike_props, y = AUC,
    color = factor(magnitude_spikes), linetype = AUC_Type
  )) +
  ...
  # --- Pseudo-trials ratio (Stimulated) ---
  ...
  range_log_stim <- range(log10(plot_tbl_pseudo_stim$PseudoRatioStim), na.rm = TRUE)
  break_vec <- seq(floor(range_log_stim[1]), ceiling(range_log_stim[2]), by = 1)

  plot_pseudo_stim <- ggplot(plot_tbl_pseudo_stim, aes(
    x = spike_props, y = PseudoRatioStim,
    color = factor(magnitude_spikes)
  )) +
    geom_line(na.rm = TRUE) +
    labs(
      x = "Proportion of Spikes",
      y = "Pseudo cells / Real cells",
      color = "Magnitude"
    ) +
    cowplot::theme_cowplot() +
    theme(
  ...
  # --- Pseudo-trials ratio (Unstimulated) ---
  ...
  range_log_uns <- range(log10(plot_tbl_pseudo_uns$PseudoRatioUnstim), na.rm = TRUE)
  break_vec_uns <- seq(floor(range_log_uns[1]), ceiling(range_log_uns[2]), by = 1)

  plot_pseudo_uns <- ggplot(plot_tbl_pseudo_uns, aes(
    x = spike_props, y = PseudoRatioUnstim,
    color = factor(magnitude_spikes)
  )) +
    geom_line(na.rm = TRUE) +
    labs(
  ...
  # --- Combine and Save ---
  combined_plot <- plot_auc + plot_pseudo_stim + plot_pseudo_uns + 
    plot_layout(ncol = 3, guides = "collect") 

  ggsave(
    filename = paste0("plots/plot_noise_", mtd, "_combined.png"),
    plot = combined_plot,
...
spike_props <- c(0, 0.05, 0.1, 0.2) 
magnitude_spikes <- c(mu_u_NR, mu_u_NR * 5, mu_u_NR * 10, mu_u_NR * 20)
N <- 2e4 #em is heavy , originally is 1e4 or 2e4                              
n_samples <- 50                        
w <- 0.5                                
n_runs <- 10
mtd_vec <- "EM"  # EM only
...
if (run_fast) {
  spike_props_active <- spike_props_active[[1]]
  magnitude_spikes_active <- magnitude_spikes_active[[1]]
  mtd_vec <- "EM"
}
for (mtd in mtd_vec) {
  print(paste0("Method: ", mtd))
  for (spike in spike_props_active) {
    ...
    for (magnitude in magnitude_spikes_active) {
      ...
      for (run in seq_len(n_runs)) {
        ...
        # Normal data 
        params <- calculate_params(base_mu_u_NR, base_mu_u_NR, f, c_u, c_s)
        dat <- simulate_data(n_samples_active, w, params, N, run)
        data_all <- dat$data
        responder_all <- dat$responder_status
        # Spiked data 
        # Spiked data (only if n_spike > 0)
        if (n_spike > 0) {
  # Identify stimulated indices among responders
  responder_indices <- seq(n_samples_active + 1, n_samples_active * 2)[responder_all == 1L]
  
  # Ensure we don't try to sample more responders than available
  n_spike <- min(round(n_samples_active * spike), length(responder_indices))
  
  if (n_spike > 0) {
    rep_ind <- sample(responder_indices, size = n_spike, replace = FALSE)
...
for (mtd in mtd_vec) {
  ...
  # ----- AUC plot -----
  plot_tbl_auc_active <- results_auc_active |>
    dplyr::filter(method == mtd) |>
    dplyr::group_by(spike_props, magnitude_spikes) |>
  ...
  plot_auc_active <- ggplot(plot_tbl_auc_active, aes(x = spike_props, y = AUC, color = factor(magnitude_spikes))) +
    geom_line(na.rm=TRUE) +
    labs(x = "Proportion of Spikes", y = "AUC", color = "Magnitude") + 
    cowplot::theme_cowplot() +
    theme(plot.background = element_rect(fill = "white", colour = "white"),
          panel.background = element_rect(fill = "white", colour = "white"))

  # ----- Pseudo-stim plot -----
  plot_tbl_pseudo_stim_active <- results_pseudo_active |>
    dplyr::filter(method == mtd) |>
    dplyr::group_by(spike_props, magnitude_spikes) |>
    dplyr::summarise(PseudoRatioStim = mean(PseudoRatioStim, na.rm = TRUE), .groups = "drop") |>
    dplyr::ungroup() |>
  ...
  max_ratio <- max(plot_tbl_pseudo_stim_active$PseudoRatioStim, na.rm = TRUE)
  min_ratio <- min(plot_tbl_pseudo_stim_active$PseudoRatioStim, na.rm = TRUE)
  break_vec <- seq(floor(log10(min_ratio)), ceiling(log10(max_ratio)), by = 1)

  plot_pseudo_stim_active <- ggplot(plot_tbl_pseudo_stim_active, aes(x = spike_props, y = PseudoRatioStim, color = factor(magnitude_spikes))) +
    geom_line(na.rm=TRUE) +
    labs(x = "Proportion of Spikes", y = "Number of Pseudo cells/Real cells", color = "Magnitude") +
    cowplot::theme_cowplot() +
  ...
  # ----- Pseudo-unstim plot -----
  plot_tbl_pseudo_uns_active <- results_pseudo_active |>
    dplyr::filter(method == mtd) |>
    dplyr::group_by(spike_props, magnitude_spikes) |>
    dplyr::summarise(PseudoRatioUnstim = mean(PseudoRatioUnstim, na.rm = TRUE), .groups = "drop") |>
    dplyr::ungroup() |>
  ...
  max_ratio_uns <- max(plot_tbl_pseudo_uns_active$PseudoRatioUnstim, na.rm = TRUE)
  min_ratio_uns <- min(plot_tbl_pseudo_uns_active$PseudoRatioUnstim, na.rm = TRUE)
  break_vec_uns <- seq(floor(log10(min_ratio_uns)), ceiling(log10(max_ratio_uns)), by = 1)

  plot_pseudo_uns_active <- ggplot(plot_tbl_pseudo_uns_active, aes(x = spike_props, y = PseudoRatioUnstim, color = factor(magnitude_spikes))) +
    geom_line(na.rm=TRUE) +
    labs(x = "Proportion of Spikes", y = "Number of Pseudo cells/Real cells", color = "Magnitude") +
    cowplot::theme_cowplot() +
    scale_y_continuous(trans = "log10",
  ...
  # ----- Round legend labels -----
  plot_auc_active <- plot_auc_active + scale_color_discrete(labels = function(x) round(as.numeric(x), 7))
  plot_pseudo_stim_active <- plot_pseudo_stim_active + scale_color_discrete(labels = function(x) round(as.numeric(x), 7))
  plot_pseudo_uns_active <- plot_pseudo_uns_active + scale_color_discrete(labels = function(x) round(as.numeric(x), 7))

  # ----- Combine all three plots vertically with shared legend -----
  combined_active <- plot_auc_active / plot_pseudo_stim_active / plot_pseudo_uns_active +
  ...
  ggsave(paste0("plots/plot_active_combined_", mtd, ".png"),
         plot = combined_active, width = 18, height = 6, dpi = 300)
}
```
...
if (run_fast) {
  spike_props <- spike_props_resting[[1]]
  magnitude_spikes <- magnitude_spikes_resting[[1]]
  mtd_vec <- "EM"
}
for (mtd in mtd_vec) {
  print(paste0("Method: ", mtd))
  for (spike in spike_props_resting) {
    ...
    for (magnitude in magnitude_spikes_resting) {
      ...
      for (run in seq_len(n_runs)) {
        ...
        dat <- simulate_data(n_samples_resting, w, params, N, run)
        data_all <- dat$data
        responder_all <- dat$responder_status
        # Spiked data 
        # Spiked data (only if n_spike > 0)
        if (n_spike > 0) {
          # only select stimulated indices, from inactivated samples
          rep_ind <- sample(
            c(
              seq(1, n_samples_resting),
              n_samples_resting + which(responder_all == 0L)
            ),
            size = n_spike, replace = FALSE
            )
          data_all <- dat$data
          data_all[rep_ind, "CYTNUM"] <- rbinom(n = n_spike, size = N, prob = magnitude)
          data_all[rep_ind, "NSUB"] <- N - data_all[["CYTNUM"]][rep_ind]
...
# --- Combine and save the 3 plots into one figure ---
for (mtd in mtd_vec) {
  # Recreate the same data frames used for plotting
  plot_tbl_auc_resting <- results_auc_resting |>
    dplyr::filter(method == mtd) |>
    dplyr::group_by(spike_props, magnitude_spikes) |>
    dplyr::summarise(AUC = mean(AUC, na.rm = TRUE), .groups = "drop") |>
    dplyr::ungroup() |>
  ...
  plot_tbl_pseudo_stim_resting <- results_pseudo_resting |>
    dplyr::filter(method == mtd) |>
    dplyr::group_by(spike_props, magnitude_spikes) |>
    dplyr::summarise(PseudoRatioStim = mean(PseudoRatioStim, na.rm = TRUE), .groups = "drop") |>
    dplyr::ungroup() |>
  ...
  plot_tbl_pseudo_uns_resting <- results_pseudo_resting |>
    dplyr::filter(method == mtd) |>
    dplyr::group_by(spike_props, magnitude_spikes) |>
    dplyr::summarise(PseudoRatioUnstim = mean(PseudoRatioUnstim, na.rm = TRUE), .groups = "drop") |>
    dplyr::ungroup() |>
  ...
  # Round magnitudes to 7 decimals for a clean legend
  plot_tbl_auc_resting$magnitude_spikes <- round(plot_tbl_auc_resting$magnitude_spikes, 7)
  plot_tbl_pseudo_stim_resting$magnitude_spikes <- round(plot_tbl_pseudo_stim_resting$magnitude_spikes, 7)
  plot_tbl_pseudo_uns_resting$magnitude_spikes <- round(plot_tbl_pseudo_uns_resting$magnitude_spikes, 7)

  # Recreate the three simplified plots (no titles)
  p1 <- ggplot(plot_tbl_auc_resting, aes(x = spike_props, y = AUC, color = factor(magnitude_spikes))) +
    geom_line(na.rm = TRUE) +
    labs(x = "Proportion of Spikes", y = "AUC", color = "Magnitude") +
    cowplot::theme_cowplot() +
    theme(plot.title = element_blank())

  p2 <- ggplot(plot_tbl_pseudo_stim_resting, aes(x = spike_props, y = PseudoRatioStim, color = factor(magnitude_spikes))) +
    geom_line(na.rm = TRUE) +
    labs(x = "Proportion of Spikes", y = "Pseudo/Real Cells", color = "Magnitude") +
    cowplot::theme_cowplot() +
    theme(plot.title = element_blank()) +
    scale_y_continuous(trans = "log10", labels = scales::comma)

  p3 <- ggplot(plot_tbl_pseudo_uns_resting, aes(x = spike_props, y = PseudoRatioUnstim, color = factor(magnitude_spikes))) +
    geom_line(na.rm = TRUE) +
    labs(x = "Proportion of Spikes", y = "Pseudo/Real Cells", color = "Magnitude") +
    cowplot::theme_cowplot() +
    theme(plot.title = element_blank()) +
  ...
  combined_plot <- patchwork::wrap_plots(p1, p2, p3, nrow = 1, guides = "collect") &
    theme(legend.position = "bottom",
  ...
  # Save combined figure
  ggsave(
    paste0("plots/plot_resting_combined_", mtd, ".png"),
    plot = combined_plot,
    width = 18, height = 6, dpi = 300
  )
}
