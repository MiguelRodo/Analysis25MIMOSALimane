  ...
  for (spike in spike_props) {
    ...
    for (magnitude in magnitude_spikes) {
      ...
      for (run in seq_len(n_runs)) {
        ...
        cat("Prepared for MIMOSA, first few rows:\n")
        print(table(E@phenoData$responder_status))
        ...
        # Compute ROC and Fisher AUC
        roc_obj <- roc(
          response = responder_all[rn_vec],
          predictor = prob_mat[, 2],
          direction = "<"
        )
        auc_val <- as.numeric(roc_obj$auc)
        auc_fisher <- fishers_auc(.data = data_all, status = responder_all, alternative = "less")
...
for (mtd in mtd_vec) {
  ...
  ggsave(paste0("plots/plot_noise_", mtd, "_pseudo_stim.png"), plot = plot_pseudo_stim, width = 6, height = 4, dpi = 300)
  ggsave(paste0("plots/plot_noise_", mtd, "_pseudo_uns.png"), plot = plot_pseudo_uns, width = 6, height = 4, dpi = 300)
}
```
...
for (mtd in mtd_vec) {
  ...
  plot_tbl_pseudo_stim_active <- results_pseudo_active |>
    ...
    dplyr::ungroup() |>
    dplyr::filter(!is.nan(PseudoRatioStim))
  max_ratio <- plot_tbl_pseudo_stim_active$PseudoRatioStim |> max(na.rm = TRUE)
  max_log <- ceiling(log10(max_ratio))
  min_ratio <- plot_tbl_pseudo_stim_active$PseudoRatioStim |> min(na.rm = TRUE)
  min_log <- floor(log10(min_ratio))
  break_vec <- seq(min_log, max_log, by = 1)
  ...
  plot_tbl_pseudo_uns_active <- results_pseudo_active |>
    ...
    dplyr::ungroup() |>
    dplyr::filter(!is.nan(PseudoRatioUnstim))
  max_ratio_uns <- plot_tbl_pseudo_uns_active$PseudoRatioUnstim |> max(na.rm = TRUE)
  max_log_uns <- ceiling(log10(max_ratio_uns))
  min_ratio_uns <- plot_tbl_pseudo_uns_active$PseudoRatioUnstim |> min(na.rm = TRUE)
  min_log_uns <- floor(log10(min_ratio_uns))
  break_vec_uns <- seq(min_log_uns, max_log_uns, by = 1)
  ...
  # Save
  ggsave(paste0("plots/plot_active_", mtd, "_auc.png"), plot = plot_auc_active, width = 6, height = 4, dpi = 300)
  ggsave(paste0("plots/plot_active_", mtd, "_pseudo_stim.png"), plot = plot_pseudo_stim_active, width = 6, height = 4, dpi = 300)
  ggsave(paste0("plots/plot_active_", mtd, "_pseudo_uns.png"), plot = plot_pseudo_uns_active, width = 6, height = 4, dpi = 300)
}
```
...
for (mtd in mtd_vec) {
  ...
  plot_tbl_auc_resting <- results_auc_resting |>
    ...
    dplyr::ungroup() |>
    dplyr::filter(!is.nan(AUC)) 
  plot_auc_resting <- ggplot(plot_tbl_auc_resting, aes(x = spike_props, y = AUC, color = factor(magnitude_spikes))) +
    geom_line(na.rm=TRUE) +
    labs(title = "AUC vs Proportion of Spikes",
        x = "Proportion of Spikes",
        y = "AUC",
  ...
  plot_tbl_pseudo_stim_resting <- results_pseudo_resting |>
    ...
    dplyr::ungroup() |>
    dplyr::filter(!is.nan(PseudoRatioStim))
  max_ratio <- plot_tbl_pseudo_stim_resting$PseudoRatioStim |> max(na.rm = TRUE)
  max_log <- ceiling(log10(max_ratio))
  min_ratio <- plot_tbl_pseudo_stim_resting$PseudoRatioStim |> min(na.rm = TRUE)
  min_log <- floor(log10(min_ratio))
  break_vec <- seq(min_log, max_log, by = 1)
  plot_pseudo_stim_resting <- ggplot(plot_tbl_pseudo_stim_resting, aes(x = spike_props, y = PseudoRatioStim, color = factor(magnitude_spikes))) +
    geom_line(na.rm=TRUE) +
    labs(title = "Pseudo-trials ratio vs Proportion of Spikes",
        x = "Proportion of Spikes",
        y = "Number of Pseudo cells/Real cells",
  ...
  plot_tbl_pseudo_uns_resting <- results_pseudo_resting |>
    ...
    dplyr::ungroup() |>
    dplyr::filter(!is.nan(PseudoRatioUnstim))
  max_ratio_uns <- plot_tbl_pseudo_uns_resting$PseudoRatioUnstim |> max(na.rm = TRUE)
  max_log_uns <- ceiling(log10(max_ratio_uns))
  min_ratio_uns <- plot_tbl_pseudo_uns_resting$PseudoRatioUnstim |> min(na.rm = TRUE)
  min_log_uns <- floor(log10(min_ratio_uns))
  break_vec_uns <- seq(min_log_uns, max_log_uns, by = 1)
  plot_pseudo_uns_resting <- ggplot(plot_tbl_pseudo_uns_resting, aes(x = spike_props, y = PseudoRatioUnstim, color = factor(magnitude_spikes))) +
    geom_line(na.rm=TRUE) +
    labs(title = "Pseudo-trials ratio vs Proportion of Spikes",
        x = "Proportion of Spikes",
        y = "Number of Pseudo cells/Real cells",
  ...
  # Save
  ggsave(
    paste0("plots/plot_resting_", mtd, "_auc.png"),
    plot = plot_auc_resting,
    width = 6, height = 4, dpi = 300
    )
  ggsave(paste0("plots/plot_resting_", mtd, "_pseudo_stim.png"), plot = plot_pseudo_stim_resting, width = 6, height = 4, dpi = 300)
  ggsave(paste0("plots/plot_resting_", mtd, "_pseudo_uns.png"), plot = plot_pseudo_uns_resting, width = 6, height = 4, dpi = 300)
}
```
