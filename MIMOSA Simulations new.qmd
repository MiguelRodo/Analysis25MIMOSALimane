...
for (mtd in mtd_vec) {
  ...
  # ----- Prepare tables and round magnitudes -----
  ...
  # ----- Create individual plots (no separate scales) -----
  p1 <- ggplot(plot_tbl_auc_active, aes(x = spike_props, y = AUC, color = factor(magnitude_spikes))) +
    geom_line(na.rm = TRUE) +
    labs(x = "Proportion of Spikes", y = "AUC", color = "Magnitude") +
    cowplot::theme_cowplot() +
    theme(plot.title = element_blank())

  p2 <- ggplot(plot_tbl_pseudo_stim_active, aes(x = spike_props, y = PseudoRatioStim, color = factor(magnitude_spikes))) +
    geom_line(na.rm = TRUE) +
  ...
  p3 <- ggplot(plot_tbl_pseudo_uns_active, aes(x = spike_props, y = PseudoRatioUnstim, color = factor(magnitude_spikes))) +
    geom_line(na.rm = TRUE) +
    labs(x = "Proportion of Spikes", y = "Pseudo / Real Cells", color = "Magnitude") +
    cowplot::theme_cowplot() +
    theme(plot.title = element_blank()) +
  ...
  # ----- Combine plots in 1 row with a single shared legend -----
  combined_active <- patchwork::wrap_plots(p1, p2, p3, nrow = 1, guides = "collect") &
    theme(
  ...
  # ----- Save figure -----
  ggsave(
    filename = paste0("plots/plot_active_combined_", mtd, ".png"),
    plot = combined_active,
    width = 18, height = 6, dpi = 300
  )
}
